...
Current lower bound $L \gets 0$\;
Initialize tree $\mathfrak{T} \gets \{ n_0\}$\;
\While{not finished}{
	$\mathfrak{S} \gets$ the set of non-leaf nodes whose children have not yet been generated\;
	\uIf{$\mathfrak{S} = \O$}{
		$\mathcal{X} \gets \mathcal{I}$ from node associated with current value of $L$\;
		\Return{$\mathcal{X}, L$}
	}
	\Else{
		$n \gets \argmax\{ v_{\mathrm{LP}}^*[n] : n \in \mathfrak{S}\}$\;
		Generate children of $n$\;
		\For{each child $n'$ of $n$}{
			$L \gets \max\{L, v_{\mathcal{I}}[n']\}$\;
		}
	}
	\For{$n''$ in $\mathfrak{T}$} {
		\lIf{$L >  v_{\mathrm{LP}}^*[n'']$}{fathom $n''$}
	}
}
\end{algorithm}
