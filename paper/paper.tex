...
\ifen
Two extensions of this problem appear amenable to future study. First, the familiar Markowitz portfolio optimization model includes an explicit risk-aversion term, whereas our model has settled for an implicit treatment of risk, namely the tradeoff between selective schools and safety schools inherent in the ``maximax'' objective function. However, it is possible to augment the objective function $v(\mathcal{X}) = \operatorname{E}[X]$ to incorporate a variance penalty $\beta \geq 0$ as follows:
\else
이 문제를 확장할 수 있는 향후 연구 방향 2가지를 제안한다. 첫번째는, 익숙한 Markowitz 포트폴리오 최적화 모형에는 명시적인 위험 회피 항이 있으며, 본 모형은 경쟁적인 학교와 안정 학교 사이의 균형으로 내포된 위험 관리 요소만 고려했다. 그러나 기본 목적함수인 $v(\mathcal{X}) = \operatorname{E}[X]$에 평균편차 페널티 $\beta \geq 0$를 다음처럼 도입할 수 있다:
\fi
\begin{align}
\begin{split}
...
미국 입학 과정의 유사한 점은 조기 전형(early decision)이다. 이는 학생이 지원하는 시기에 합격하면 진학한다고 약속하는 지원 방법이다. 조기 전형으로 지원하면 입학할 관심을 시그널할 수 있으므로 합격 확률을 높일 수 있다. 반면에 학교가 학생을 모집하려고 장학금을 수여할 동기가 없어질 수 있으므로 학생의 효용 추정이 달라진다.
\fi
