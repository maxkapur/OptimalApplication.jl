...
\subsection{Pseudopolynomial-time dynamic programs}

\subsubsection{Dynamic program based on application costs}
...
\subsection{A polynomial-time approximation scheme}
By rounding the $f_j$ values down to the nearest multiple of $1/K$, where $K \in \mathbb{N}$, Algorithm \ref{ellisDP2} yields a polynomial-time approximation scheme for Ellis's problem. Here we assume only that is each $t_j$ is integral. 

\begin{algorithm}[H] 
%\DontPrintSemicolon
\caption{Polynomial-time approximation scheme for Ellis's problem.} \label{ellisPTAS}
...
$K \gets \bigl\lceil\,{m}/{\sqrt[m]{\varepsilon f_m}}\,\bigr\rceil$\;
Round each $f_j$ down to the nearest multiple of $1/K$\;
\Return{} the output of Algorithm \ref{ellisDP2}
\end{algorithm}
...
\begin{proof}
The initialization steps trivial. By rounding, we ensure that $D \leq K^m$; therefore, the time required by Algorithm \ref{ellisDP2} is
\begin{equation}
O\left(Dm + m\log m \right) = 
\end{equation}
\end{proof}
